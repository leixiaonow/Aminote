...
public class AmigoMenuBuilder implements Menu {
    ...
    private ArrayList<AmigoMenuItemImpl> mItems;
    private boolean mItemsChangedWhileDispatchPrevented = false;
    private ArrayList<AmigoMenuItemImpl> mNonActionItems;
    private boolean mOptionalIconsVisible = false;
    private CopyOnWriteArrayList<WeakReference<AmigoMenuPresenter>> mPresenters = new CopyOnWriteArrayList();
    private boolean mPreventDispatchingItemsChanged = false;
    private boolean mQwertyMode;
    private final Resources mResources;
    private boolean mShortcutsVisible;
    private ArrayList<AmigoMenuItemImpl> mTempShortcutItemList = new ArrayList();
    private ArrayList<AmigoMenuItemImpl> mVisibleItems;

    public interface Callback {
        boolean onMenuItemSelected(AmigoMenuBuilder amigoMenuBuilder, MenuItem menuItem);

        void onMenuModeChange(AmigoMenuBuilder amigoMenuBuilder);
    }

    public interface ItemInvoker {
        boolean invokeItem(AmigoMenuItemImpl amigoMenuItemImpl);
    }

    public AmigoMenuBuilder(Context context) {
    ...
    public AmigoMenuBuilder setDefaultShowAsAction(int defaultShowAsAction) {
        this.mDefaultShowAsAction = defaultShowAsAction;
        return this;
    }

    public void addMenuPresenter(AmigoMenuPresenter presenter) {
        this.mPresenters.add(new WeakReference(presenter));
        presenter.initForMenu(this.mContext, this);
        this.mIsActionItemsStale = true;
    }

    public void removeMenuPresenter(AmigoMenuPresenter presenter) {
        Iterator i$ = this.mPresenters.iterator();
        while (i$.hasNext()) {
            WeakReference<AmigoMenuPresenter> ref = (WeakReference) i$.next();
            AmigoMenuPresenter item = (AmigoMenuPresenter) ref.get();
    ...
    private void dispatchPresenterUpdate(boolean cleared) {
        if (!this.mPresenters.isEmpty()) {
            ...
            while (i$.hasNext()) {
                WeakReference<AmigoMenuPresenter> ref = (WeakReference) i$.next();
                AmigoMenuPresenter presenter = (AmigoMenuPresenter) ref.get();
                if (presenter == null) {
                    this.mPresenters.remove(ref);
                } else {
    ...
    private boolean dispatchSubMenuSelected(AmigoSubMenuBuilder SubMenu) {
        if (this.mPresenters.isEmpty()) {
            return false;
        }
    ...
    public void saveActionViewStates(Bundle outStates) {
        ...
        for (int i = 0; i < itemCount; i++) {
            ...
            if (item.hasSubMenu()) {
                ((AmigoSubMenuBuilder) item.getSubMenu()).saveActionViewStates(outStates);
            }
        }
        if (viewStates != null) {
    ...
    public void restoreActionViewStates(Bundle states) {
        if (states != null) {
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                if (item.hasSubMenu()) {
                    ((AmigoSubMenuBuilder) item.getSubMenu()).restoreActionViewStates(states);
                }
            }
    ...
    private MenuItem addInternal(int group, int id, int categoryOrder, CharSequence title) {
        ...
        AmigoMenuItemImpl item = new AmigoMenuItemImpl(this.mContext, this, group, id, categoryOrder, ordering, title, this.mDefaultShowAsAction);
    ...
    public SubMenu addSubMenu(int group, int id, int categoryOrder, CharSequence title) {
        AmigoMenuItemImpl item = (AmigoMenuItemImpl) addInternal(group, id, categoryOrder, title);
        AmigoSubMenuBuilder SubMenu = new AmigoSubMenuBuilder(this.mContext, this, item);
    ...
    public void removeGroup(int group) {
        ...
        if (i >= 0) {
            ...
            while (true) {
                ...
                if (numRemoved >= maxRemovable || ((AmigoMenuItemImpl) this.mItems.get(i)).getGroupId() != group) {
                    onItemsChanged(true);
                } else {
    ...
    void setExclusiveItemChecked(MenuItem item) {
        ...
        for (int i = 0; i < N; i++) {
            MenuItem curItem = (AmigoMenuItemImpl) this.mItems.get(i);
    ...
    public void setGroupCheckable(int group, boolean checkable, boolean exclusive) {
        int N = this.mItems.size();
        for (int i = 0; i < N; i++) {
            AmigoMenuItemImpl item = (AmigoMenuItemImpl) this.mItems.get(i);
    ...
    public boolean hasVisibleItems() {
        ...
        for (int i = 0; i < size; i++) {
            if (((AmigoMenuItemImpl) this.mItems.get(i)).isVisible()) {
                return true;
            }
        }
        return false;
    ...
    public int findItemIndex(int id) {
        int size = size();
        for (int i = 0; i < size; i++) {
            if (((AmigoMenuItemImpl) this.mItems.get(i)).getItemId() == id) {
                return i;
            }
        }
        return -1;
    ...
    public int findGroupIndex(int group, int start) {
        ...
        for (int i = start; i < size; i++) {
            if (((AmigoMenuItemImpl) this.mItems.get(i)).getGroupId() == group) {
                return i;
            }
        }
        return -1;
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(AmigoWidgetResource.getIdentifierByBool(this.mContext, "amigo_config_showMenuShortcutsWhenKeyboardPresent")))) {
            z = false;
        }
    ...
    boolean dispatchMenuItemSelected(AmigoMenuBuilder Menu, MenuItem item) {
    ...
    private static int findInsertIndex(ArrayList<AmigoMenuItemImpl> items, int ordering) {
        for (int i = items.size() - 1; i >= 0; i--) {
            if (((AmigoMenuItemImpl) items.get(i)).getOrdering() <= ordering) {
                return i + 1;
            }
        }
        return 0;
    ...
    public boolean performShortcut(int keyCode, KeyEvent event, int flags) {
        AmigoMenuItemImpl item = findItemWithShortcutForKey(keyCode, event);
    ...
    void findItemsWithShortcutForKey(List<AmigoMenuItemImpl> items, int keyCode, KeyEvent event) {
        ...
        if (event.getKeyData(possibleChars) || keyCode == 67) {
            ...
            for (int i = 0; i < N; i++) {
                AmigoMenuItemImpl item = (AmigoMenuItemImpl) this.mItems.get(i);
                if (item.hasSubMenu()) {
                    ((AmigoMenuBuilder) item.getSubMenu()).findItemsWithShortcutForKey(items, keyCode, event);
                }
    ...
    AmigoMenuItemImpl findItemWithShortcutForKey(int keyCode, KeyEvent event) {
        ArrayList<AmigoMenuItemImpl> items = this.mTempShortcutItemList;
        items.clear();
        ...
        int size = items.size();
        if (size == 1) {
            return (AmigoMenuItemImpl) items.get(0);
        }
        boolean qwerty = isQwertyMode();
        for (int i = 0; i < size; i++) {
            AmigoMenuItemImpl item = (AmigoMenuItemImpl) items.get(i);
            char shortcutChar = qwerty ? item.getAlphabeticShortcut() : item.getNumericShortcut();
    ...
    public boolean performItemAction(MenuItem item, int flags) {
        AmigoMenuItemImpl itemImpl = (AmigoMenuItemImpl) item;
        ...
        if (itemImpl.hasCollapsibleActionView()) {
            ...
            if (!itemImpl.hasSubMenu()) {
                itemImpl.setSubMenu(new AmigoSubMenuBuilder(getContext(), this, itemImpl));
            }
            AmigoSubMenuBuilder SubMenu = (AmigoSubMenuBuilder) itemImpl.getSubMenu();
    ...
    void onItemVisibleChanged(AmigoMenuItemImpl item) {
        this.mIsVisibleItemsStale = true;
        onItemsChanged(true);
    }

    void onItemActionRequestChanged(AmigoMenuItemImpl item) {
        this.mIsActionItemsStale = true;
        onItemsChanged(true);
    }

    ArrayList<AmigoMenuItemImpl> getVisibleItems() {
    ...
    public void flagActionItems() {
        if (this.mIsActionItemsStale) {
            ...
            if (flagged) {
                ...
                ArrayList<AmigoMenuItemImpl> visibleItems = getVisibleItems();
                int itemsSize = visibleItems.size();
                for (int i = 0; i < itemsSize; i++) {
                    AmigoMenuItemImpl item = (AmigoMenuItemImpl) visibleItems.get(i);
    ...
    ArrayList<AmigoMenuItemImpl> getActionItems() {
        flagActionItems();
        return this.mActionItems;
    }

    public ArrayList<AmigoMenuItemImpl> getNonActionItems() {
        flagActionItems();
    ...
    protected AmigoMenuBuilder setHeaderTitleInt(CharSequence title) {
        setHeaderInternal(0, title, 0, null, null);
        return this;
    }

    protected AmigoMenuBuilder setHeaderTitleInt(int titleRes) {
        setHeaderInternal(titleRes, null, 0, null, null);
        return this;
    }

    protected AmigoMenuBuilder setHeaderIconInt(Drawable icon) {
        setHeaderInternal(0, null, 0, icon, null);
        return this;
    }

    protected AmigoMenuBuilder setHeaderIconInt(int iconRes) {
        setHeaderInternal(0, null, iconRes, null, null);
        return this;
    }

    protected AmigoMenuBuilder setHeaderViewInt(View view) {
    ...
    public AmigoMenuItemImpl getExpandedItem() {
